...
void MX_FREERTOS_Init(void) {
  ...
  /* USER CODE BEGIN RTOS_THREADS */
	...
	mon_list[9].h = (TaskHandle_t)canTaskHandle;
	mon_list[9].stack_words = (uint16_t)(canTask_attributes.stack_size / sizeof(StackType_t));
	mon_list[9].alias = "CAN";

	Monitor_RegisterTaskInfo(mon_list, 10);
	
	IWDG_TaskInit(NULL, controlTaskHandle, NULL, monitorTaskHandle, sensorTaskHandle);

...
/* 单通道编码器只能测频率，方向由最近一次非零电机指令估计。 */
static volatile int8_t s_dirL = 1;
static volatile int8_t s_dirR = 1;
...
{
    if (left_sign > 0) s_dirL = 1;
    else if (left_sign < 0) s_dirL = -1;

    if (right_sign > 0) s_dirR = 1;
    else if (right_sign < 0) s_dirR = -1;
}
...
float EncoderCap_GetLeftMmps(void)
{
    ...
    return hz_to_speed(EncoderCap_GetLeftHz()) * (float)s_dirL;
}
/**
 * @brief 获取右轮线速度（mm/s）
 */
float EncoderCap_GetRightMmps(void)
{
    ...
    return hz_to_speed(EncoderCap_GetRightHz()) * (float)s_dirR;
}
/* ============================== 差速运动学 ============================== */

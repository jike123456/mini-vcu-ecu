 ...
 */
static uint8_t ms6dsv_write(uint8_t addr, uint8_t reg, uint16_t len,
                            const uint8_t *dat)
{
    uint16_t i;
 ...
 *              MS6DSV_EACK: IIC通讯ACK错误，函数执行失败
 */
static uint8_t ms6dsv_read(uint8_t addr, uint8_t reg, uint16_t len,
                           uint8_t *dat)
{
    ms6dsv_iic_start();
    ms6dsv_iic_send_byte((addr << 1) | 0);
...
{
    return ms6dsv_write(ms6dsv_iic_addr, reg, len, bufp);
}

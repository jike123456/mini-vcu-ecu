...
bool UplinkProto_SendVirtualCan(proto_uplink_t *tx, uint8_t seq,
                               uint16_t can_id, uint8_t dlc,
                               const uint8_t data[8])
{
    ...
    if ((tx == NULL) || (data == NULL) || (dlc > 8u) || (s_txBusy != 0u))
    {
        return false;
    }

    buffer = tx->send_buffer;
    buffer[0] = PROTO_SOF0;
    buffer[1] = PROTO_SOF1;
    buffer[2] = PROTO_VER;
    buffer[3] = PROTO_MSG_VCAN_TX;
    buffer[4] = PROTO_FLAG_DEFAULT;
    buffer[5] = seq;
    buffer[6] = (uint8_t)(PROTO_VCAN_PAYLOAD_LEN & 0xFFu);
    buffer[7] = (uint8_t)(PROTO_VCAN_PAYLOAD_LEN >> 8u);

    buffer[8] = (uint8_t)(can_id & 0xFFu);
    buffer[9] = (uint8_t)(can_id >> 8u);
    buffer[10] = dlc;
    for (index = 0u; index < 8u; index++)
    {
        buffer[11u + index] = (index < dlc) ? data[index] : 0u;
    }

    crc = Proto_Crc16CcittFalse(&buffer[PROTO_OFF_VER],
                                (uint16_t)(PROTO_LEN_HDR_FIXED + PROTO_VCAN_PAYLOAD_LEN));
    buffer[19] = (uint8_t)(crc & 0xFFu);
    buffer[20] = (uint8_t)(crc >> 8u);
    buffer[21] = PROTO_EOF0;
    buffer[22] = PROTO_EOF1;
    frame_length = 23u;

    return uart1_send_dma(buffer, frame_length);
}

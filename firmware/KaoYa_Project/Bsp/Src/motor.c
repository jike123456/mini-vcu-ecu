...
void Motor_ApplyPwmL(int16_t pwmL)
{
    ...
    s_pwmL_out = pwmL;
    EncoderCap_SetDirectionHint((pwmL > 0) ? 1 : ((pwmL < 0) ? -1 : 0), 0);
...
void Motor_ApplyPwmR(int16_t pwmR)
{
    ...
    s_pwmR_out = pwmR;
    EncoderCap_SetDirectionHint(0, (pwmR > 0) ? 1 : ((pwmR < 0) ? -1 : 0));
...
#if MOTOR_CFG_SYNC_ENC_SIM
    /* 同步模拟编码器速度（便于单独验证 TIM 输出/捕获链路） */

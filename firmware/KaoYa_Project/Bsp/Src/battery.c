...
typedef union
{
    ...
    volatile uint16_t samples[BAT_ADC_BUF_LEN];
} BatteryAdcBuffer_t;

/*
 * HAL_ADC_Start_DMA() uses a uint32_t pointer for all DMA widths.  The DMA is
 * configured for half-word transfers, while this union guarantees the address
 * still satisfies the API's 32-bit alignment requirement.
 */
static BatteryAdcBuffer_t s_batteryAdcBuf;
...
void Battery_Init(void)
{
	...
	/* 启动 ADC DMA 采样 */
    (void)HAL_ADC_Start_DMA(&hadc3,
                            &s_batteryAdcBuf.dma_alignment,
                            BAT_ADC_BUF_LEN);
...
uint16_t Battery_VoltageMvGet() 
{
    ...
    {
        adc_sum += s_batteryAdcBuf.samples[i];
    }

...
typedef enum
{
    ...
    PROTO_MSG_VCAN_RX      = 0x02,   /* PC 仿真节点 -> MCU：一帧 CAN 报文 */

    /* MCU -> Host */
    PROTO_MSG_STATUS       = 0x81,   /* 状态上报 */
    PROTO_MSG_VCAN_TX      = 0x82,   /* MCU -> PC 仿真节点：一帧 CAN 报文 */
    PROTO_MSG_ACK          = 0x80,   /* 通用 ACK（可选） */
} proto_msg_id_t;

/* 虚拟 CAN payload: can_id(u16 LE) + dlc(u8) + data[8] */
#define PROTO_VCAN_PAYLOAD_LEN      (11u)

...
/** 将协议物理量安全收敛到 int16，避免强转溢出。 */
static int16_t sensor_float_to_i16(float value)
{
 ...
 * 避免其他任务读到“撕裂数据”。
 */
static void SensorSnap_Set(const proto_status_t *s)
{
    /* 防御：空指针检查 */
    if (!s) return;
...
void KaoYaApp_SensorTask(void *argument) {
...
#endif
    imu_ready = (ret == 0u);
    FaultManager_SetObserved(FAULT_INJECT_IMU,
                             (uint8_t)(imu_ready ? 0u : 1u));
    if (!imu_ready)
    {
    ...
    for (;;) {
		...
		int16_t vx_mmps = sensor_float_to_i16(vx_f);
		int16_t vy_mmps = sensor_float_to_i16(vy_f);
		int16_t vz_mrad = sensor_float_to_i16(wz_f);

		/* ==================== IMU 数据 ==================== */
		
		/* 查询 IMU 数据就绪标志；读失败不阻塞整个 ECU。 */
        if (imu_ready && (lsm6dsv16x_flag_data_ready_get(&ms6dsv, &drdy) != 0))
        {
            imu_ready = false;
            FaultManager_SetObserved(FAULT_INJECT_IMU, 1u);
            LOGERROR_T("IMU", "runtime communication failed");
        }

		/* 加速度数据就绪 */
        if (imu_ready && drdy.drdy_xl)
        {
		...
		/* ==================== 单位整形 ==================== */
        int16_t temp_x100 = sensor_float_to_i16(temperature_degc * 100.0f);

        /* 传感器输出 mdps；协议单位是 0.01 deg/s，因此除以 10。 */
        int16_t gx_cdegps = sensor_float_to_i16(angular_rate_mdps[0] / 10.0f);
        int16_t gy_cdegps = sensor_float_to_i16(angular_rate_mdps[1] / 10.0f);
        int16_t gz_cdegps = sensor_float_to_i16(angular_rate_mdps[2] / 10.0f);

        int16_t ax_mg = sensor_float_to_i16(acceleration_mg[0]);
        int16_t ay_mg = sensor_float_to_i16(acceleration_mg[1]);
        int16_t az_mg = sensor_float_to_i16(acceleration_mg[2]);
		...
		/* 六轴：角速度 0.01 deg/s */
		LOGKAOYA_T("IMU", "【角速度】(0.01dps) gx：%d gy：%d gz：%d", (int)gx_cdegps, (int)gy_cdegps, (int)gz_cdegps);

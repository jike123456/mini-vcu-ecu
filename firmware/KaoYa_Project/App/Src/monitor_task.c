...
volatile uint32_t g_hb_can       = 0;  // cantask的心跳
...
void KaoYaApp_MonitorTask(void *argument)
{
    ...
    for (;;) {
		...
		/* 5) DTC NvM：仅在非驱动状态、有变更且达到限频时写 Flash。 */
        {
            VehicleStateSnapshot_t vehicle = VehicleState_GetSnapshot();
            FaultManager_Service((uint32_t)(xTaskGetTickCount() * portTICK_PERIOD_MS));
            NvMDtc_Service((uint32_t)(xTaskGetTickCount() * portTICK_PERIOD_MS),
                           vehicle.state != VCU_STATE_DRIVE);
        }

		/* 序号递增 */
        s_mon_seq++;
		IWDG_Heartbeat(IWDG_ID_MONITOR);
...
void Monitor_OnStackOverflow(TaskHandle_t xTask, const signed char *task_name)
{
    ...
    if (task_name) 
	{
        LOGERROR_T("MON", "STACK OVERFLOW: task=%s", (const char *)task_name);
    } 
	else
	{		

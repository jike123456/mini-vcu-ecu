...
#define CAN_LBL_X           650u
#define CAN_VAL_X           696u
#define CAN_VAL_W           94u
                                  ...
                                  uint16_t text_color)
{
    ...
    x_end_u32 = (uint32_t)x + (uint32_t)w;
    y_end_u32 = (uint32_t)y + (uint32_t)h;
    x_end = (x_end_u32 > UINT16_MAX) ? UINT16_MAX : (uint16_t)x_end_u32;
    y_end = (y_end_u32 > UINT16_MAX) ? UINT16_MAX : (uint16_t)y_end_u32;

    /* 外框 */
    lcd_draw_rectangle(x, y, x_end, y_end, border_color);

    /* 内部背景（留 1px 边） */
    if (w > 2 && h > 2) {
        x_inner = (x < UINT16_MAX) ? (uint16_t)(x + 1u) : UINT16_MAX;
        y_inner = (y < UINT16_MAX) ? (uint16_t)(y + 1u) : UINT16_MAX;
        lcd_fill(x_inner, y_inner, (uint16_t)(x_end - 1u),
                 (uint16_t)(y_end - 1u), bg_color);
    }

    /* 填充条 */
    inner_w = (w > 2u) ? (uint16_t)(w - 2u) : 0u;
    inner_h = (h > 2u) ? (uint16_t)(h - 2u) : 0u;
    fill_w = (uint16_t)((uint32_t)inner_w * (uint32_t)pct / 100u);

    if (fill_w > 0 && inner_h > 0) {
        uint32_t fill_x_end = (uint32_t)x + 1u + (uint32_t)fill_w;
        uint32_t fill_y_end = (uint32_t)y + 1u + (uint32_t)inner_h;
        x_inner = (x < UINT16_MAX) ? (uint16_t)(x + 1u) : UINT16_MAX;
        y_inner = (y < UINT16_MAX) ? (uint16_t)(y + 1u) : UINT16_MAX;
        lcd_fill(x_inner, y_inner,
                 (fill_x_end > UINT16_MAX) ? UINT16_MAX : (uint16_t)fill_x_end,
                 (fill_y_end > UINT16_MAX) ? UINT16_MAX : (uint16_t)fill_y_end,
                 fill_color);
    }
    ...
    x_end_u32 += 8u;
    y_end_u32 = (uint32_t)h + 6u;
    text_x = (x_end_u32 > UINT16_MAX) ? UINT16_MAX : (uint16_t)x_end_u32;
    text_y = (y >= 2u) ? (uint16_t)(y - 2u) : 0u;
    text_h = (y_end_u32 > UINT16_MAX) ? UINT16_MAX : (uint16_t)y_end_u32;
    lcd_show_string(text_x, text_y, 60u, text_h, 16u, buf, text_color);
}
...
static void UI_DrawStatic(void)
{
    ...
    lcd_show_string(TMP_VAL_X, UI_STATUS_Y + 12, TMP_VAL_W, 24, 16, "----C", C_TEXT);
		...
		lcd_show_string(CMD_LBL_X, UI_STATUS_Y + 12,50, 24, 16, "VCU:", C_TEXT);
		lcd_show_string(CMD_VAL_X, UI_STATUS_Y + 12,CMD_VAL_W, 24, 16, "--------", C_TEXT);

        lcd_show_string(CAN_LBL_X, UI_STATUS_Y + 12, 46, 24, 16, "CAN:", C_TEXT);
        lcd_show_string(CAN_VAL_X, UI_STATUS_Y + 12, CAN_VAL_W, 24, 16, "INIT", C_TEXT);
    ...
    for (uint32_t i = 0; i < ROW_COUNT; i++) {
        ...
        lcd_show_string(COL_TASK_X, y + 8, 140, 24, 16, s_rows[i].name, C_TEXT);
...
static void UI_UpdateDynamicReal(void)
{
    ...
    /* 3) 顶部：Temp（℃*100 -> xx.xxC） */
    {
		...
		/* 显示 VCU 业务状态机，不是 FreeRTOS 任务状态。 */
		{
            VehicleStateSnapshot_t vehicle = VehicleState_GetSnapshot();
            const char *state_text = "INIT";
            uint16_t state_color = C_WARNBAR;
            ...
            case VCU_STATE_DRIVE:
                ...
                state_color = C_OKBAR;
                break;
            case VCU_STATE_SAFE_STOP:
                state_text = "SAFE";
                state_color = C_ERRBAR;
                break;
        ...
        /* CAN self-test state is visible even when the UART terminal misses logs. */
        {
            CanSelfTestStats_t can_stats;
            const char *can_text = "INIT";
            uint16_t can_color;

            CanTask_GetSelfTestStats(&can_stats);
            if (!can_stats.initialized) {
                switch (can_stats.init_status) {
                case 1U: can_text = "QUEUE"; break;
                case 2U: can_text = "SLEEP"; break;
                case 3U: can_text = "ENTER"; break;
                case 4U: can_text = "LEAVE"; break;
                default: can_text = "INIT"; break;
                }
                can_color = C_ERRBAR;
            } else if (can_stats.pass_count > 0U) {
                can_text = "PASS";
                can_color = C_OKBAR;
            } else if (can_stats.fail_count > 0U) {
                if ((can_stats.bus_tx_ok == 0U) && (can_stats.bus_tx_fail > 0U)) {
                    can_text = "TXBUSY";
                } else if ((can_stats.bus_tx_ok > 0U) && (can_stats.bus_rx_ok == 0U)) {
                    can_text = "NO RX";
                } else {
                    can_text = "DATA";
                }
    ...
    for (uint32_t i = 0; i < ROW_COUNT; i++) {
        ...
        /* state */
        eTaskState task_state = eTaskGetState(t.h);
        const char *ststr = UI_StateStr(task_state);
        lcd_show_string(COL_STATE_X, y + 8, 140, 24, 16, ststr, C_TEXT);
        ...
        if (s_rows[i].hb_ptr) {
            ...
            if (cur != s_rows[i].last_hb) {
                ...
                /* 事件驱动任务在等待队列/通知时没有新心跳，这是正常状态。 */
                lcd_show_string(COL_HB_X, y + 8, 70, 24, 16, "WAIT", C_TEXT);
            } else {

...
#define IWDG_TIMEOUT_MONITOR_MS     (2500u)
...
void IWDG_WatchEnable(IwdgId_t id, bool en)
{
    ...
    uint8_t was_enabled = g_watchEnable[id];
    g_watchEnable[id] = en ? 1u : 0u;

    /* 启用监控时，立即刷新心跳
     * 防止刚启用就被历史时间判定超时
     */
    if (en && (was_enabled == 0u))
	{
		g_lastKickTick[id] = (uint32_t)xTaskGetTickCount();
    }
}

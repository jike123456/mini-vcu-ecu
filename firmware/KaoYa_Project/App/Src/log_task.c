 ...
 * ========================= */
/** @brief 日志输出等级 */
#define LOG_RUNTIME_LEVEL LOG_LVL_INFO
...
{
    switch (lvl) {
    case LOG_LVL_KAOYA: return "KAOYA";
...
void HAL_UART_TxCpltCallback(UART_HandleTypeDef *huart)
{
	...
	else if (huart && huart->Instance == USART1) {
...
/**
 * @brief HAL UART 异常回调
 * @note 释放 USART1 上报通道，避免一次异常后永久 busy。
 */
void HAL_UART_ErrorCallback(UART_HandleTypeDef *huart)
{
    if (huart && huart->Instance == USART1) {
        UplinkProto_OnUartError(huart);
    }
}

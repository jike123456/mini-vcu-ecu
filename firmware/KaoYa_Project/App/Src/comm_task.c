 ...
 *  - 通过临界区整体拷贝，避免撕裂读
 */
static VehicleCanRxContext_t s_vcan_rx = {0};
...
{
    return CommandBroker_GetLatest();
}

static void publish_command(const CmdVel_t *command)
{
    CommandBroker_Publish(command);
    if (g_controlTaskHandle != NULL)
    {
        (void)xTaskNotify(g_controlTaskHandle, CTRL_NOTIF_NEW_CMD, eSetBits);
    }
}
...
static void on_frame_cb(const proto_frame_t *f, void *user)
{
    ...
    if (f->msg_id == PROTO_MSG_CMD_VEL)
    {
		...
		v.valid     = true;
		v.source    = CMD_SOURCE_UART_LEGACY;

		publish_command(&v);
    }
    else if (f->msg_id == PROTO_MSG_VCAN_RX)
    {
        uint16_t can_id;
        uint8_t dlc;
        VehicleCanCommand_t can_command;
        VehicleCanDecodeStatus_t decode_status;
        CmdVel_t command = {0};

        if (f->len != PROTO_VCAN_PAYLOAD_LEN) return;

        can_id = proto_rd_u16(f->payload, 0u);
        dlc = f->payload[2];
        if (can_id == UDS_CAN_ID_PHYSICAL_REQUEST)
        {
            UdsServer_ProcessCanFrame(&f->payload[3], dlc,
                                      (uint32_t)(xTaskGetTickCount() * portTICK_PERIOD_MS));
            return;
        }
        if (can_id == VEHICLE_CAN_ID_TEST_FAULT)
        {
            const uint8_t *data = &f->payload[3];
            if ((dlc == 8u) && (data[1] == 0xA5u) &&
                (data[2] == 0u) && (data[3] == 0u) &&
                (data[4] == 0u) && (data[5] == 0u) &&
                ((data[6] & 0xF0u) == 0u) &&
                (VehicleCan_Crc8SaeJ1850(data, 7u) == data[7]))
            {
                FaultManager_SetInjectionMask(data[0]);
                LOGINFO_T("FAULT", "injection mask=0x%02X", (unsigned)data[0]);
            }
            return;
        }
        if (can_id != VEHICLE_CAN_ID_COMMAND) return;

        decode_status = VehicleCan_DecodeCommand(&s_vcan_rx,
                                                  &f->payload[3],
                                                  dlc,
                                                  &can_command);
        if (decode_status != VEHICLE_CAN_DECODE_OK)
        {
            uint32_t now_ms = (uint32_t)(xTaskGetTickCount() * portTICK_PERIOD_MS);
            if (decode_status == VEHICLE_CAN_DECODE_CRC)
            {
                DtcManager_Report(DTC_ID_COMMAND_CRC, true, now_ms);
            }
            else if (decode_status == VEHICLE_CAN_DECODE_COUNTER)
            {
                DtcManager_Report(DTC_ID_COMMAND_COUNTER, true, now_ms);
            }
            else if (decode_status == VEHICLE_CAN_DECODE_VALUE)
            {
                DtcManager_Report(DTC_ID_COMMAND_VALUE, true, now_ms);
            }
            LOGKAOYA_T("VCAN", "CMD reject reason=%u crc=%lu cnt=%lu val=%lu",
                       (unsigned)decode_status,
                       (unsigned long)s_vcan_rx.crc_errors,
                       (unsigned long)s_vcan_rx.counter_errors,
                       (unsigned long)s_vcan_rx.value_errors);
            return;
        }

        if ((can_command.ignition != 0u) && (can_command.gear != 0u))
        {
            command.vx_mmps = can_command.target_vx_mmps;
            command.wz_mradps = can_command.target_wz_mradps;
        }
        command.vy_mmps = 0;
        command.ignition = can_command.ignition;
        command.gear = can_command.gear;
        command.drive_enable = ((can_command.ignition != 0u) &&
                                (can_command.gear != 0u));
        command.rx_tick = xTaskGetTickCount();
        command.valid = true;
        command.source = CMD_SOURCE_VIRTUAL_CAN;
        publish_command(&command);
        DtcManager_Report(DTC_ID_COMMAND_CRC, false,
                          (uint32_t)(command.rx_tick * portTICK_PERIOD_MS));
        DtcManager_Report(DTC_ID_COMMAND_COUNTER, false,
                          (uint32_t)(command.rx_tick * portTICK_PERIOD_MS));
        DtcManager_Report(DTC_ID_COMMAND_VALUE, false,
                          (uint32_t)(command.rx_tick * portTICK_PERIOD_MS));

        if ((s_vcan_rx.accepted % 50u) == 1u)
        {
            LOGKAOYA_T("VCAN", "CMD PASS n=%lu vx=%d wz=%d alive=%u",
                       (unsigned long)s_vcan_rx.accepted,
                       (int)command.vx_mmps,
                       (int)command.wz_mradps,
                       (unsigned)can_command.alive_counter);
        }
    }

...
#define CTRL_CFG_CMD_TIMEOUT_MS            (100u)
/** 判定“停稳”的轮速阈值（mm/s） */
 ...
 * @brief 读取控制环唯一轮速反馈源。
 *
 * 纯软件回归时，PID 直接读电机模型，不依赖 PB8->PA15 / PB9->PB3
 * 两根实物跳线。切换到真实编码器模式后才读 TIM2 输入捕获。
 */
static inline void ctrl_read_wheel_feedback(float *vL_mmps, float *vR_mmps)
{
#if MOTOR_CFG_USE_SIM
    Motor_GetWheelSpeedMmps(vL_mmps, vR_mmps);
#else
    if (vL_mmps != NULL) *vL_mmps = EncoderCap_GetLeftMmps();
    if (vR_mmps != NULL) *vR_mmps = EncoderCap_GetRightMmps();
#endif
}
...
void KaoYaApp_ControlTask(void *argument)
{
    ...
    for (;;)
    {
        ...
        if (mode == CTRL_MODE_IDLE)
        {
			...
			{	
				/* 读取最新指令 */
                ref_cmd = CommandBroker_GetLatest();
                last_cmd_ms = ctrl_now_ms();
				s_cmd_timeout = false;
                VehicleState_OnValidCommand(ref_cmd.drive_enable, last_cmd_ms);
                SafetyManager_SetInhibit(SAFETY_INHIBIT_CMD_TIMEOUT, false);
                SafetyManager_SetInhibit(SAFETY_INHIBIT_NOT_DRIVE,
                                         !ref_cmd.drive_enable);
                DtcManager_Report(DTC_ID_COMMAND_TIMEOUT, false, last_cmd_ms);
				/* 切换到 RUN 模式 */
                mode = CTRL_MODE_RUN;
        ...
        FaultManager_Service(tnow);
        if ((tnow - last_cmd_ms) > CTRL_CFG_CMD_TIMEOUT_MS) 
				{
			bool drive_was_requested = ref_cmd.drive_enable;
            ref_cmd.valid = false;
			s_cmd_timeout = true;
            if (drive_was_requested)
            {
                /* 仅当上一条有效指令要求驱动时，丢帧才是安全故障。 */
                VehicleState_OnCommandTimeout(tnow);
                SafetyManager_SetInhibit(SAFETY_INHIBIT_CMD_TIMEOUT, true);
                DtcManager_Report(DTC_ID_COMMAND_TIMEOUT, true, tnow);
            }
            else
            {
                /* 点火关闭/STANDBY 时节点下线是正常工况，不生成超时 DTC。 */
                SafetyManager_SetInhibit(SAFETY_INHIBIT_CMD_TIMEOUT, false);
                DtcManager_Report(DTC_ID_COMMAND_TIMEOUT, false, tnow);
            }
        }
        ...
        if (ref_cmd.valid && ref_cmd.drive_enable) 
				{
						/* 根据 vx / wz 反算左右轮目标速度 */
        ...
        float vR_meas_mmps = 0.0f;
				ctrl_read_wheel_feedback(&vL_meas_mmps, &vR_meas_mmps);
        {
            VehicleStateSnapshot_t vehicle = VehicleState_GetSnapshot();
            DtcManager_UpdateContext(ref_cmd.vx_mmps,
                                     ref_cmd.wz_mradps,
                                     ctrl_clampi16((int32_t)vL_meas_mmps, -32768, 32767),
                                     ctrl_clampi16((int32_t)vR_meas_mmps, -32768, 32767),
        ...
        if (ref_cmd.valid && ref_cmd.drive_enable && VehicleState_DriveAllowed())
        {
#if PID_KAOYATEACH_ENABLE
...
#endif
			pwmL = PID_Update(&pidL, vL_ref_mmps, vL_meas_mmps, CTRL_CFG_DT_SEC);
...
#endif
			pwmR = PID_Update(&pidR, vR_ref_mmps, vR_meas_mmps, CTRL_CFG_DT_SEC);
		...
		(void)SafetyManager_ApplyMotorPwm(pwmL, pwmR);
